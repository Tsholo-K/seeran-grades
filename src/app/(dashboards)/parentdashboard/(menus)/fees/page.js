...
const ChildMenu = ({ child, bottom_border=false }) => {
  return (
    <>
      <Link href={`/fees/${child.student_number}`} className="flex py-2 gap-5 px-3 justify-start w-full cursor-pointer md:hover:text-blue-700">
    ...
    <div className="w-full relative grid grid-cols-1 place-content-center">
      ...
      <div className="z-[1]">
        ...
        <div className="pb-16 " >
          ...
          <p className=" text-sm w-[90%] text-gray-400 text-center mx-auto pt-2">check outstanding balances and/or invoices</p>
        </div>
        {/* children menu */}
            ...
            <>
              {
                ( data.students.length > 1  &&  index !== data.students.length - 1) 
                  ? <ChildMenu key={index} child={child} bottom_border={true}/>
                  : <ChildMenu key={index} child={child}/>
              }
            </>
          ))
          }
